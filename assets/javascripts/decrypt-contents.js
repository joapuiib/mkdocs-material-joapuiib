...
/* Decrypt content of a page */
function decrypt_config_action(username_input, password_input, encrypted_config, key_from_storage=false) {
    let key=false;
    let keys_from_keystore=false;
    ...
    let content = false;
    if (key) {
        content = decrypt_content_from_bundle(key, encrypted_config.innerHTML);
    }
    if (content !== false) {
...
function decryptor_reaction(key_or_keys, password_input, decrypted_content, fallback_used=false) {
    if (key_or_keys) {
        ...
        let encrypted_something = {'mkdocs-encrypted-toc': ['nav', 'class']};
...
function init_decryptor() {
    ...
    password_input.addEventListener('keypress', function(event) {
        if (event.key === "Enter") {
            ...
            inside_time_range = decrypt_config_action(
